<template>
    <div>
        <s-card title="代码片段" class="mt-10">
            <el-button @click="dialogFormVisible=true">新增搜索</el-button>
            <el-button @click="createCodepage">生成并复制代码</el-button>
            <el-collapse v-show="collapseSerch.length!==0" v-model="activeName">
                <el-collapse-item v-for="(item,index) in collapseSerch" :key="index" :name="item.id">
                    <div slot="title" class="d-flex">
                        <span>{{ item.label?item.label:"点击配置" }}</span>
                    </div>
        ...
        <el-dialog title="配置本地枚举数组" :visible.sync="dialogFormVisible1" width="40%">
            ...
            <div>
                <el-form v-for="(item,index) in localSelectData" :key="index" :inline="true">
...
export default {
    ...
    methods: {
        ...
        //=====================================生成代码====================================//
        createCodepage() {
            const search = [...this.collapseSerch].map(item => {
                return {
                    label: item.label,
                    vModel: item.vModel,
